...
if do_correct:
    for i, obi in enumerate(obtags):
        ...
        if obi.split('.')[0] not in cfgs_no_CO:
...
#              '13CO.tav.contsub.corrected'
    ...
    for i in range(len(cfgs)):
        for sufix in sufixes:
            if sufix.split('.')[0]=='12CO' and cfgs[i] in cfgs_no_CO:
                continue
...
if do_clean_lines:
    ...
    for i,linei in enumerate(flines):
        ...
        ms_image=[]
        for cfgi in cfgs[:]:
            mslinei  ='{}.{}.cal.bary.{}.tav.contsub.corrected.ms'.format(system['target'],cfgi,tags[i] )
        ...
        image_name='{}.combined.cal.bary.{}'.format(system['target'], tags[i])
        ...
        print('nchan: {}, vi: {} km/s, width:'.format(nchan, vi)+ width_inp)
