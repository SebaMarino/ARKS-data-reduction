...
- ACA observed with 21 executions in October  2022, April, May and June.
- 12mSB was not executed
- 12mLB observed with 11 executions between Dec 2023 and July 2024.
...
cfgs=['12mLB', 'ACA']
cfgs_no_13CO=[]
obref_f='12mLB.obs1' # for flux
obref_o='12mLB.obs1' # for offsets
...
results['fflux-{}'.format(obref_f.split('.')[0]+'.obs1')]={'median':1.0}

#results['fflux-{}'.format(obref2)]={'median':1.0}
...
if do_correct:
    for i, obi in enumerate(obtags[:]):
        print(i, obi)
        print('RA correction = {:.4f} DEC correction = {:.4f}'.format(results['deltaRA-{}'.format(obi)]['median']-results['deltaRA-{}'.format(obref_o)]['median'], results['deltaDec-{}'.format(obi)]['median']-results['deltaDec-{}'.format(obref_o)]['median']))
        
        
#         # with SMG subtraction
#         apply_corrections('../reduced/{}.{}.cal.bary.continuum.fav.tav.ms'.format(target, obi),
#                           dRA=results['deltaRA-{}'.format(obi)]['median']-results['deltaRA-{}'.format(obref_o)]['median'],
#                           dDec=results['deltaDec-{}'.format(obi)]['median']-results['deltaDec-{}'.format(obref_o)]['median'],
#                           fsigma=results['fsigma-{}'.format(obi)]['median'],
#                           flux_offset=results['fflux-{}'.format(obi)]['median']/results['fflux-{}'.format(obref_f)]['median'],
...
#                           cfg=obi)
        ...
        if obi.split('.')[0] not in cfgs_no_13CO:
...
#               'continuum.fav.tav.corrected',
    ...
    ## get ref direction
    msmd.open('{}.{}.cal.bary.{}.ms'.format(target, obref_o, sufixes[0]))
    ...
    for i in range(len(cfgs)):
        for sufix in sufixes:
            if (sufix.split('.')[0]=='13CO') and cfgs[i] in cfgs_no_13CO: # sufix.split('.')[0]=='12CO' or 
                continue
