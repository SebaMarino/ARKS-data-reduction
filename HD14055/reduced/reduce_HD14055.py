...
### system parameters
system={'target'      : 'HD14055', # used to name files
        'field_name'  : 'HD14055', # field name in ms
...
#         'Radius'      : 17.64, # arcsec, 600 au as 2x outer radius maximum radius. This is larger than PB so the radius was set to FWHM of Antenna primary beam    

        'rv'       : 5.11, # km/s, radial velocity in barycentric reference frame from Gaia DR3
...
if do_concat:
    ...
    msfiles=[]
    msconcat='{}.{}.cal.bary.continuum.fav.tav.ms'.format(system['target'], cfg)
    for i in range(Nms):
        msfiles.append('{}.{}.obs{}.cal.bary.continuum.fav.tav.ms'.format(system['target'], cfg,i+1 ))
    os.system('rm -r '+msconcat)
    concat(msfiles, msconcat)
...
if do_concat:
    ...
    msfiles=[]
    msconcat='{}.{}.cal.bary.continuum.fav.tav.v2.ms'.format(system['target'], cfg)
    for i in range(Nms):
        msfiles.append('{}.{}.obs{}.cal.bary.continuum.fav.tav.ms'.format(system['target'], cfg,i+1 ))
    os.system('rm -r '+msconcat)
    concat(msfiles, msconcat)
