...
- 12m  band 6 observed with 5 execution blocks in Apr 2017
...
cfgs=['b7.12mLB', 'b7.12mMB', 'b7.12mSB', 'b7.ACA', 'b6.12m']
cfgs_no_CO=['b7.12mLB', 'b7.12mSB', 'b6.12m']
obref='b7.12mLB.obs1'
obref2='b6.12m.obs1'
...
if do_correct:
    for i, obi in enumerate(obtags):
        ...
        apply_corrections('../reduced/{}.{}.cal.bary.continuum.fav.ms'.format(target, obi),
                          ...
                          extractvis=False,
                          cfg=obi)
        if obi.split('.')[0]+'.'+obi.split('.')[1] not in cfgs_no_CO:
